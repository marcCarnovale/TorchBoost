...
def test_progressive_classifier_learns_rates_and_records_soft_count():
    ...
    x = r.normal(size=(180, 5)).astype("float32")
    ...
    cfg = ProgressiveConfig(
        ...
        stage_updates=6,
        batch_size=64,
        cart_value_updates=2,
        patience_stages=4,
        learn_tree_rates=True,
        tree_rate_l2=1e-4,
        tree_count_pressure=1e-4,
        tree_count_start_stage=2,
        random_state=3,
    )
    m = ProgressiveTreeClassifier(cfg).fit(
        x[:140], y[:140], eval_set=(x[140:], y[140:])
    )
